...
class DPStarshipView: BaseView {
    ...
    func setupData(starshipsData: [StarshipDataModel]) {
        viewModel.starshipsData = starshipsData
        listCollectionView.reloadData()
        
        noDataLabel.isHidden = !viewModel.starshipsData.isEmpty
        listCollectionView.isHidden = viewModel.starshipsData.isEmpty
    }
...
extension DPStarshipView: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel.starshipsData.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = viewModel.starshipsData[indexPath.row].data {
            cell.setupData(title: data.name)
        } else {
            cell.showSkeleton(isShow: true)
            viewModel.getDetailStarship(id: viewModel.starshipsData[indexPath.row].id) { isSuccess, error in
                if isSuccess {
                    if let data = self.viewModel.starshipsData[indexPath.row].data {
                        cell.setupData(title: data.name)
                    }
